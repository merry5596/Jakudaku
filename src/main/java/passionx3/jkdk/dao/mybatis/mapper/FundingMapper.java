...
public interface FundingMapper {
  ...
  // 여기부터 조건 검색
	@Select("SELECT I.ITEMID, I.USERID AS PRODUCERID, A.ALIAS AS PRODUCERNAME, I.THEMEID, T.NAME AS THEMENAME, I.NAME, I.UPLOADDATE, I.PRICE, I.LIKENUM,"
			...
			+ " FROM ITEM I, FUNDINGITEM F, THEME T, ACCOUNT A"
			+ " WHERE I.ITEMID = F.ITEMID AND I.THEMEID = T.THEMEID AND I.USERID = A.USERID AND FINISHDATE >= #{today} AND i.name LIKE '%' ||  #{keyword} || '%' ORDER BY UPLOADDATE DESC")
	List<Funding> getFundingItemListOrderByUploadDate(String keyword, String today);

	@Select("SELECT I.ITEMID, I.USERID AS PRODUCERID, A.ALIAS AS PRODUCERNAME, I.THEMEID, T.NAME AS THEMENAME, I.NAME, I.UPLOADDATE, I.PRICE, I.LIKENUM,"
			+ " I.THUMBNAIL1, I.THUMBNAIL2, I.THUMBNAIL3, I.ISFORSALE, I.DESCRIPTION, I.APPROVAL, TO_CHAR(F.FINISHDATE, 'YYYY/MM/DD HH24:MI:SS') AS FINISHDATE, F.PURCHASEQUANTITY, F.TARGETQUANTITY"
			+ " FROM ITEM I, FUNDINGITEM F, THEME T, ACCOUNT A"
			+ " WHERE I.ITEMID = F.ITEMID AND I.THEMEID = T.THEMEID AND I.USERID = A.USERID AND FINISHDATE >= #{today} AND i.name LIKE '%' ||  #{keyword} || '%' ORDER BY LIKENUM DESC")
	List<Funding> getFundingItemListOrderByLikeNum(String keyword, String today);
   
	@Select("SELECT I.ITEMID, I.USERID AS PRODUCERID, A.ALIAS AS PRODUCERNAME, I.THEMEID, T.NAME AS THEMENAME, I.NAME, I.UPLOADDATE, I.PRICE, I.LIKENUM,"
			+ " I.THUMBNAIL1, I.THUMBNAIL2, I.THUMBNAIL3, I.ISFORSALE, I.DESCRIPTION, I.APPROVAL, TO_CHAR(F.FINISHDATE, 'YYYY/MM/DD HH24:MI:SS') AS FINISHDATE, F.PURCHASEQUANTITY, F.TARGETQUANTITY"
			+ " FROM ITEM I, FUNDINGITEM F, THEME T, ACCOUNT A"
			+ " WHERE I.ITEMID = F.ITEMID AND I.THEMEID = T.THEMEID AND I.USERID = A.USERID AND FINISHDATE >= #{today} AND i.name LIKE '%' ||  #{keyword} || '%' ORDER BY FINISHDATE")
