...
public interface ItemMapper {
	@Insert("INSERT INTO item (itemId, themeid, name, uploadDate, price, likeNum, thumbnail1, thumbnail2, thumbnail3, isforsale, description, approval, userid) "
			+ "values (#{item.itemId}, #{item.themeId}, #{item.name}, sysdate, #{item.price}, 0, "
				+ "#{item.thumbnail1}, #{item.thumbnail2}, #{item.thumbnail3}, 1, #{item.description}, 0, #{item.producerId})")
	...
	@Update("UPDATE item "
			+ "SET themeid = #{item.themeid}, price = #{item.price}, thumbnail1 = #{item.thumbnail1}, thumbnail2 = #{item.thumbnail1=2}, thumbnail3 = #{item.thumbnail3}, "
